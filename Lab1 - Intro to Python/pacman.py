...
class Pacman:
    ...
    def __init__(self, matrix, food_counter, width, height):
    ...
    def get_random_position(self):
        ...
        if self._MOVES[random_num] == 'UP':
            ...
            x = self.player.x + 1
            y = self.player.y
            return [x if x < self.height else self.height - 1, y]
            ...
            x = self.player.x
            y = self.player.y + 1
            return [x, y if y < self.width else self.width - 1]
...
def main():
    ...
    pacman = Pacman(matrix, food_counter, width, height)
    pacman.play_game()
